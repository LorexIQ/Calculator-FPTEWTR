class Calculate(object):
    ...
    def calculate_without_reinf(self, data):
        ...
        try:
            ...
            e0 = (L_x * (L_x + L_y)) / u - x0
            self.__solution_progress_lessr.append('$u=2L_x+L_y=2%s+%s=%s$' % tuple([round(i, 2) for i in (L_x, L_y, u)]))
            ...
            Fe0 = N * e0 / 1000
            M -= Fe0
            self.__solution_progress_lessr.append('$M=M-Fe_0=%s-%s=%s$' % tuple([round(i, 2) for i in (M_loc / 2, Fe0, M)]))
            ...
            if N * 1000 / u + M * 10 ** 6 / W_b <= R_bt * h0:
                ...
                self.__solution_progress_lessr.append('$W_b=(a+h_0)(\\frac{a+h_0}{3}+b+h_0) = (%s+%s)(\\frac{%s+%s}{3}+%s+%s) = %s$' % tuple([round(i, 2) for i in (a, h0, a, h0, b, h0, W_b)]))
                M = M_loc / 2
                self.__solution_progress_lessr.append('$M=\\frac{M_{sup}+M_{inf}}{2}=\\frac{%s+%s}{2}=%s$' % tuple([round(i, 2) for i in (M_sup, M_inf, M)]))
                self.__solution_progress_lessr.append('$\\frac{F}{u}+\\frac{M}{W_b} \\leq R_{bt}h_0 ; \\frac{%s}{%s}+\\frac{%s}{%s}=%s \\leq %s*%s=%s$' % tuple([round(i, 2) for i in (N*1000, u, M*10**6, W_b, N * 1000 / u + M * 10 ** 6 / W_b, R_bt, h0, R_bt * h0)]))
                if  N * 1000 / u + M * 10 ** 6 / W_b <= R_bt * h0:
    ...
    def calculate_with_reinf(self, data):
        ...
        try:
            ...
            self.__solution_progress_inr.append('$u=2(a+b+2h_0)=2(%s+%s+2*%s)=%s$' % tuple([round(i, 2) for i in (a, b, h0, u)]))
            self.__solution_progress_inr.append('$W_{b,x}=(a+h_0)(\\frac{a+h0}{3}+b+h_0)=(%s+%s)(\\frac{%s+%s}{3}+%s+%s)=%s$' % tuple([round(i, 2) for i in (a, h0, a, h0, b, h0, W_bx)]))
            self.__solution_progress_inr.append('$W_{b,x}=(b+h_0)(\\frac{b+h0}{3}+a+h_0)=(%s+%s)(\\frac{%s+%s}{3}+%s+%s)=%s$' % tuple([round(i, 2) for i in (b, h0, b, h0, a, h0, W_by)]))
            M_x = (M_xsup + M_xinf) / 2
            M_y = (M_ysup + M_yinf) / 2
            s_w_count = h0 // s_w
            ...
            if (F / u) + (M_x / W_bx) + (M_y / W_by) > R_bt * h0:
                ...
                if 0.25 * R_bt * h0 > q_sw:
                    q_sw = 0
                self.__solution_progress_inr.append('$\\frac{F}{u} + \\frac{M_x}{W_{b,x}} + \\frac{M_y}{W_{b,y}}=frac{%s}{%s} + \\frac{%s}{%s}} + \\frac{%s}{%s}=%s ; R_{bt}h_0 + 0.8q_{sw}=%s%s+ 0.8%s =%s$' % tuple([round(i, 2) for i in (F, u, M_x, W_bx, M_y, W_by, (F / u) + (M_x / W_bx) + (M_y / W_by), R_bt, h0, q_sw, R_bt * h0 + 0.8 * q_sw) ]))
                if (F / u) + (M_x / W_bx) + (M_y / W_by) <  R_bt * h0 + 0.8 * q_sw:
                    ...
                    self.__solution_progress_inr.append('$u=2(a+b+2h_0)=2(%s+%s+2*%s)=%s$' % tuple([round(i, 2) for i in (a_new, b_new, h0, u_new)]))
                    self.__solution_progress_inr.append('$W_{b,x}=(a+h_0)(\\frac{a+h0}{3}+b+h_0)=(%s+%s)(\\frac{%s+%s}{3}+%s+%s)=%s$' % tuple([round(i, 2) for i in (a_new, h0, a_new, h0, b_new, h0, W_bx_new)]))
                    self.__solution_progress_inr.append('$W_{b,x}=(b+h_0)(\\frac{b+h0}{3}+a+h_0)=(%s+%s)(\\frac{%s+%s}{3}+%s+%s)=%s$' % tuple([round(i, 2) for i in (b_new, h0, b_new, h0, a_new, h0, W_by_new)]))
                    self.__solution_progress_in.append('$\\frac{F}{u} + \\frac{M_x}{W_{b,x}} + \\frac{M_y}{W_{b,y}}=\\frac{%s}{%s} + \\frac{%s}{%s}} + \\frac{%s}{%s}=%s ; R_{bt}h_0 + 0.8q_{sw}=%s%s =%s$' % tuple([round(i, 2) for i in (F, u_new, M_x, W_bx_new, M_y, W_by_new, ((F * 10 ** 3) / u_new) + ((M_x * 10 ** 6) / W_bx_new) + ((M_y * 10 ** 6) / W_by_new), R_bt, h0, R_bt * h0)]))
