class Calculate(object):
    ...
    def сalculate_without_reinf(self, h, a, b, N, M_sup, M_inf, x0, R_bt, h0=None):
        ...
        if N*1000/u + M*10**6/W_b <= R_bt*h0:
            ...
            M = M_loc/2
            if N*1000/u + M*10**6/W_b <= R_bt*h0:
                return 'The cross-sectional strength is ensured. Without rods.'
            else:
                return 'The cross-section sleep rate is not guaranteed.'
        else:
            return 'The cross-section sleep rate is not guaranteed.'


    def сalculate_with_reinf(self, h, a, b, N, M_xsup, M_ysup, M_xinf, M_yinf, s_w, R_bt, h0=None):
        if h0 is None:
            h0 = h - 30
        ...
        if (F / u) + (M_x / W_bx) + (M_y / W_by) > R_bt * h0:
            return 'Strength is not ensured, the separation area should be increased.'
        else:
            ...
            if (F / u) + (M_x / W_bx) + (M_y / W_by) < R_bt * h0 + 0.8 * q_sw:  
                ...
                if ((F * 10 ** 3) / u_new) + ((M_x * 10 ** 6) / W_bx_new) + \
                    ...
                    return return_line + 'The cross-sectional strength is ensured. Members:% s; Distance between bars:% s;' % \
